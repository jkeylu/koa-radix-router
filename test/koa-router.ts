  ...
  it('runs subrouter middleware after parent', function (done) {
    ...
    var subrouter = new Router()
      .use(function (ctx, next) {
        (<any>ctx).msg = 'subrouter';
        return next();
      })
      .get('/', function (ctx) {
        ctx.body = { msg: (<any>ctx).msg };
      });
    var router = new Router()
      .use(function (ctx, next) {
        (<any>ctx).msg = 'router';
    ...
    it('responds with user-provided throwable using the "throw" and "methodNotAllowed" options', function (done) {
      ...
      app.use(router.allowedMethods({
        ...
        methodNotAllowed: function () {
          ...
          (<any>notAllowedErr).type = 'custom';
          (<any>notAllowedErr).statusCode = 405;
          (<any>notAllowedErr).body = {
    ...
    it('responds with user-provided throwable using the "throw" and "notImplemented" options', function (done) {
      ...
      app.use(router.allowedMethods({
        ...
        notImplemented: function () {
          ...
          (<any>notImplementedErr).type = 'custom';
          (<any>notImplementedErr).statusCode = 501;
          (<any>notImplementedErr).body = {
  ...
  describe('Router#[verb]()', function () {
    ...
    */
    ...
    it('enables route chaining', function () {
      var router = new Router();
      methods.forEach(function (method) {
        (<any>router)[method]('/', function () {}).should.equal(router);
      });
    });
  ...
  describe('Router#redirect()', function () {
    ...
    */
    ...
    it('redirects using route names', function (done) {
      ...
      router.get('/', function () {});
      router.get('/sign-up-form', function () {});
      router.redirect('/', '/sign-up-form');
      request(http.createServer(app.callback()))
        .post('/')
        .expect(301)
        .end(function (err, res) {
  ...
  */
  ...
  describe('router.routes()', function () {
    it('should return composed middleware', function (done) {
      ...
      var middlewareA: Koa.Middleware = function (ctx, next) {
        middlewareCount++;
        return next();
      };
      var middlewareB: Koa.Middleware = function (ctx, next) {
        middlewareCount++;
        return next();
      };
